/* =========================================================
   Duru's Geschiedenis (HAVO 3) — Proeftoets 5 (20 vragen)
   Hoofdstuk 1: De Eerste Wereldoorlog — 1.5 Neutraal Nederland, Grondwet 1917 & De Stijl
   ========================================================= */
(function () {
  "use strict";

  DURU.registerExamen({
    id: "ex-h3-geschiedenis-5",
    titel: "Proeftoets 5 — 1.5 Neutraal Nederland, Grondwet 1917 & De Stijl",
    vak: "Geschiedenis · Hoofdstuk 1",
    hoofdstuk: 1,
    hoofdstukTitel: "Hoofdstuk 1 — De Eerste Wereldoorlog (1900–1920)",
    icoon: "🇳🇱",
    duurMin: 20,
    vragen: [
      {
        id: "ex5_v1",
        type: "mc",
        vraag: "Wat was het standpunt van Nederland tijdens de Eerste Wereldoorlog?",
        opties: [
          "Nederland bleef neutraal (onpartijdig) en deed niet mee aan de gevechten.",
          "Nederland vocht mee aan de zijde van Duitsland.",
          "Nederland vocht mee aan de zijde van Engeland.",
          "Nederland werd in augustus 1914 direct door Duitsland bezet."
        ],
        antwoord: 0,
        uitleg: "Nederland stelde zich neutraal op en hoefde daardoor niet mee te vechten aan het front."
      },
      {
        id: "ex5_v2",
        type: "invul",
        vraag: "Hoeveel Nederlandse militairen hield de regering tijdens de mobilisatie in juli 1914 paraat aan de grenzen?",
        antwoord: "200.000|200000|200.000 militairen",
        uitleg: "Er werden 200.000 militairen gemobiliseerd om de grenzen te bewaken."
      },
      {
        id: "ex5_v3",
        type: "mc",
        vraag: "Hoeveel Belgische vluchtelingen staken in het najaar van 1914 de grens over naar het neutrale Nederland?",
        opties: [
          "10.000",
          "Ruim 1.000.000 (één miljoen)",
          "100.000",
          "5.000.000"
        ],
        antwoord: 1,
        uitleg: "Uit angst voor Duitse terreur vluchtten ruim één miljoen Belgen naar Nederland."
      },
      {
        id: "ex5_v4",
        type: "invul",
        vraag: "Hoe noemden de mensen het levensgevaarlijke elektrische grenshek onder hoogspanning dat Duitsland eind 1914 tussen België en Nederland aanlegde?",
        antwoord: "Draad des Doods|de draad des doods|draden des doods",
        uitleg: "De 'Draad des Doods' hield vluchtelingen en spionnen tegen via een hek onder hoogspanning."
      },
      {
        id: "ex5_v5",
        type: "waaronwaar",
        vraag: "De Duitse veldmaarschalk Von Moltke zag de Rotterdamse haven als een 'luchtpijp' waardoor Duitsland overzeese goederen kon blijven importeren.",
        antwoord: true,
        uitleg: "Waar! Duitsland importeerde in het begin via het neutrale Rotterdam koffie, tabak en graan."
      },
      {
        id: "ex5_v6",
        type: "mc",
        vraag: "Waarom ontstond er vanaf 1916 in Nederland grote schaarste aan voedsel en steenkool?",
        opties: [
          "Omdat al het voedsel door de VS in beslag werd genomen.",
          "Omdat Nederlandse boeren weigerden te werken.",
          "Door zeemijnen, duikbootaanvallen en de geallieerde handelsblokkade kwamen er steeds minder goederen Rotterdam binnen.",
          "Omdat de fabrieken in Nederland afbrandden."
        ],
        antwoord: 2,
        uitleg: "Handelsblokkades en duikbootaanvallen sneden de aanvoer van grondstoffen en voedsel af."
      },
      {
        id: "ex5_v7",
        type: "mc",
        vraag: "Wat hield het overheidsstelsel van 'distributie' in?",
        opties: [
          "Iedereen mocht onbeperkt spullen meenemen uit de supermarkt.",
          "Eten werd alleen nog aan militairen gegeven.",
          "Alle winkels werden gesloten.",
          "Schaarse basisproducten gingen 'op de bon'; je had distributiebonnen nodig om vastgestelde hoeveelheden te kopen."
        ],
        antwoord: 3,
        uitleg: "Rantsoenering via bonnen zorgde voor een meer gelijke verdeling van schaarse goederen."
      },
      {
        id: "ex5_v8",
        type: "invul",
        vraag: "Hoe noemen we de illegale plek waar mensen die het konden betalen stiekem voedsel en goederen kochten zonder bonnen?",
        antwoord: "zwarte markt|de zwarte markt",
        uitleg: "Op de zwarte markt werden schaarse spullen tegen woekerprijzen verhandeld."
      },
      {
        id: "ex5_v9",
        type: "waaronwaar",
        vraag: "De oorlogsdreiging versterkte in Nederland de nationale eenheid en hielp politieke tegenstellingen te verzachten.",
        antwoord: true,
        uitleg: "Waar! Politieke partijen werkten samen om grote nationale problemen op te lossen."
      },
      {
        id: "ex5_v10",
        type: "mc",
        vraag: "Welke twee grote politieke strijdpunten werden in de Grondwetsherziening van 1917 tegelijk opgelost?",
        opties: [
          "De kiesrechtkwestie (algemeen mannenkiesrecht) en de schoolstrijd (gelijke financiering van bijzonder en openbaar onderwijs).",
          "De afschaffing van het leger en de toetreding tot de EU.",
          "De sluiting van de grens en de afschaffing van belasting.",
          "Het vrouwenkiesrecht en de afschaffing van de monarchie."
        ],
        antwoord: 0,
        uitleg: "Mannen kregen algemeen kiesrecht en bijzondere (gelovige) scholen kregen voortaan evenveel overheidsgeld."
      },
      {
        id: "ex5_v11",
        type: "invul",
        vraag: "In welk jaar kregen ook vrouwen in Nederland definitief algemeen kiesrecht (actief vrouwenkiesrecht)?",
        antwoord: "1919",
        uitleg: "Twee jaar na het mannenkiesrecht (1917) kregen vrouwen in 1919 kiesrecht."
      },
      {
        id: "ex5_v12",
        type: "mc",
        vraag: "Wie was de socialistische SDAP-leider die in november 1918 een mislukte oproep deed tot een socialistische revolutie in Nederland?",
        opties: [
          "Abraham Kuyper",
          "Pieter Jelles Troelstra",
          "Herman Gorter",
          "Johan Thorbecke"
        ],
        antwoord: 1,
        uitleg: "Troelstra dacht dat de tijd rijp was voor een revolutie, maar hij kreeg amper steun ('de vergissing van Troelstra')."
      },
      {
        id: "ex5_v13",
        type: "waaronwaar",
        vraag: "In reactie op Troelstra's revolutie-oproep kwamen duizenden mensen op het Malieveld in Den Haag bijeen om hun steun aan koningin Wilhelmina te betuigen.",
        antwoord: true,
        uitleg: "Waar! Het volk liet massaal hun aanhankelijkheid aan het Koningshuis zien."
      },
      {
        id: "ex5_v14",
        type: "mc",
        vraag: "Hoe heette de vernieuwende Nederlandse kunstbeweging die in 1917 werd opgericht?",
        opties: [
          "Cobra",
          "De Nachtwacht",
          "De Stijl",
          "Het Impressionisme"
        ],
        antwoord: 2,
        uitleg: "De Stijl was de beroemde kunstenaarsbeweging gericht op eenvoud, harmonie en abstractie."
      },
      {
        id: "ex5_v15",
        type: "mc",
        vraag: "Welke drie primaire kleuren gebruikte de kunstbeweging De Stijl voornamelijk in hun kunstwerken?",
        opties: [
          "Roze, bruin en cyan",
          "Groen, oranje en paars",
          "Zwart, grijs en goud",
          "Rood, geel en blauw"
        ],
        antwoord: 3,
        uitleg: "De Stijl gebruikte rood, geel en blauw, gecombineerd met wit, grijs en zwart."
      },
      {
        id: "ex5_v16",
        type: "invul",
        vraag: "Wie was de beroemdste Nederlandse schilder van De Stijl, bekend van strakke zwarte lijnen en het schilderij 'Victory Boogie Woogie'?",
        antwoord: "Piet Mondriaan|Mondriaan|piet mondriaan",
        uitleg: "Piet Mondriaan was de bekendste schilder van De Stijl."
      },
      {
        id: "ex5_v17",
        type: "mc",
        vraag: "Wie was de bekende architect en meubelmaker van De Stijl die strakke meubels met primaire kleuren ontwierp (zoals de rood-blauwe stoel)?",
        opties: [
          "Gerrit Rietveld",
          "Hendrik Berlage",
          "Bram van der Leck",
          "Vincent van Gogh"
        ],
        antwoord: 0,
        uitleg: "Gerrit Rietveld paste de ideeën van De Stijl toe in meubels en architectuur."
      },
      {
        id: "ex5_v18",
        type: "waaronwaar",
        vraag: "Abstracte kunst is kunst waarvan je direct precies kunt zien welke persoon of welk landschap is afgebeeld.",
        antwoord: false,
        uitleg: "Onwaar! Bij abstracte kunst kun je niet direct zien wat het voorstelt; het gebruikt lijnen, vormen en kleuren om gevoel/harmonie uit te drukken (tegenovergestelde van figuratieve kunst)."
      },
      {
        id: "ex5_v19",
        type: "mc",
        vraag: "Waar verwijst de naam van Mondriaans beroemde schilderij 'Victory Boogie Woogie' (1944) naar?",
        opties: [
          "Naar de Nederlandse overwinning in de Eerste Wereldoorlog.",
          "Naar de aanstaande overwinning op nazi-Duitsland en zijn favoriete boogiewoogiemuziek.",
          "Naar de invoering van de euro in 2002.",
          "Naar het spoorwegsysteem in Engeland."
        ],
        antwoord: 1,
        uitleg: "Mondriaan werkte aan Victory Boogie Woogie in New York tijdens WO2; de naam verwijst naar de komende geallieerde overwinning."
      },
      {
        id: "ex5_v20",
        type: "open",
        vraag: "Leg uit waarom de Grondwetsherziening van 1917 leidde tot grote tevredenheid bij zowel confessionele (gelovige) als socialistische partijen in Nederland.",
        sleutelwoorden: [
          "socialisten/algemeen kiesrecht/mannenkiesrecht/kiesrechtkwestie",
          "confessionelen/gelovigen/bijzonder onderwijs/schoolstrijd/financiering"
        ],
        minTreffers: 1,
        modelantwoord: "De socialisten waren tevreden omdat hun eis van algemeen mannenkiesrecht (kiesrechtkwestie) werd ingewilligd. De confessionelen (gelovigen) waren tevreden omdat hun gelovige (bijzondere) scholen voortaan evenveel overheidsgeld kregen als openbare scholen (schoolstrijd).",
        uitleg: "Een uitruil (pacificatie): kiesrecht voor links/socialisten, gelijke schoolfinanciering voor rechts/confessionelen."
      }
    ]
  });
})();
