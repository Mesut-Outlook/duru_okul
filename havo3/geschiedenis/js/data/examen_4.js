/* =========================================================
   Duru's Geschiedenis (HAVO 3) — Proeftoets 4 (20 vragen)
   Hoofdstuk 1: De Eerste Wereldoorlog — 1.4 De nieuwe kaart van Europa & Turkije
   ========================================================= */
(function () {
  "use strict";

  DURU.registerExamen({
    id: "ex-h3-geschiedenis-4",
    titel: "Proeftoets 4 — 1.4 De nieuwe kaart van Europa & Stichting van Turkije",
    vak: "Geschiedenis · Hoofdstuk 1",
    hoofdstuk: 1,
    hoofdstukTitel: "Hoofdstuk 1 — De Eerste Wereldoorlog (1900–1920)",
    icoon: "🗺️",
    duurMin: 20,
    vragen: [
      {
        id: "ex4_v1",
        type: "mc",
        vraag: "Waar kwamen de Geallieerde overwinnaars in 1919 bijeen om de vredesvoorwaarden op te stellen?",
        opties: [
          "In Parijs (Versailles)",
          "In Londen",
          "In Berlijn",
          "In Washington"
        ],
        antwoord: 0,
        uitleg: "De vredesconferentie vond in 1919 plaats in en rond Parijs."
      },
      {
        id: "ex4_v2",
        type: "mc",
        vraag: "Wie waren de 'Grote Drie' die de beslissingen namen tijdens de Vredesconferentie in Parijs?",
        opties: [
          "Lenin (Rusland), Bismarck (Duitsland) en Churchill (VK)",
          "Wilson (VS), Clemenceau (Frankrijk) en Lloyd George (Groot-Brittannië)",
          "Atatürk (Turkije), Wilhelmina (NL) en Roosevelt (VS)",
          "Hitler (Duitsland), Mussolini (Italië) en Stalin (Sovjet-Unie)"
        ],
        antwoord: 1,
        uitleg: "Woodrow Wilson (VS), Georges Clemenceau (Frankrijk) en David Lloyd George (VK) waren de belangrijkste leiders."
      },
      {
        id: "ex4_v3",
        type: "waaronwaar",
        vraag: "Duitsland mocht in 1919 meepraten en onderhandelen over de tekst van het Vredesverdrag van Versailles.",
        antwoord: false,
        uitleg: "Onwaar! Duitsland werd uitgesloten van de onderhandelingen en mocht alleen het definitieve dictaat ondertekenen."
      },
      {
        id: "ex4_v4",
        type: "mc",
        vraag: "Wat hield het 'alleenschuld-artikel' (Artikel 231) in de Vrede van Versailles in?",
        opties: [
          "Alle Europese landen waren even schuldig.",
          "Frankrijk en Engeland moesten de schade van Duitsland vergoeden.",
          "Alleen Duitsland en zijn bondgenoten waren schuldig aan het uitbreken van de oorlog.",
          "De schuld werd kwijtgescholden."
        ],
        antwoord: 2,
        uitleg: "Duitsland kreeg de volledige schuld en verantwoordelijkheid voor alle oorlogsschade opgelegd."
      },
      {
        id: "ex4_v5",
        type: "invul",
        vraag: "Welk grensgebied moest Duitsland in 1919 teruggeven aan Frankrijk?",
        antwoord: "Elzas-Lotharingen|Elzas Lotharingen",
        uitleg: "Elzas-Lotharingen, dat Duitsland in 1871 had veroverd, ging terug naar Frankrijk."
      },
      {
        id: "ex4_v6",
        type: "mc",
        vraag: "Tot hoeveel man werd het Duitse leger maximaal beperkt volgens het Verdrag van Versailles?",
        opties: [
          "50.000 man",
          "1.000.000 man",
          "500.000 man",
          "100.000 man"
        ],
        antwoord: 3,
        uitleg: "Het Duitse leger werd teruggebracht tot hooguit 100.000 man zonder zware wapens."
      },
      {
        id: "ex4_v7",
        type: "waaronwaar",
        vraag: "In de Vrede van Versailles raakte Duitsland alle overzeese kolonies kwijt.",
        antwoord: true,
        uitleg: "Waar! Alle Duitse kolonies in Afrika en Azië werden onder beheer van de Volkenbond gesteld."
      },
      {
        id: "ex4_v8",
        type: "invul",
        vraag: "Hoe heette de internationale organisatie die in 1919 werd opgericht om toekomstige oorlogen te voorkomen?",
        antwoord: "Volkenbond|De Volkenbond|volkenbond",
        uitleg: "De Volkenbond werd op initiatief van president Wilson opgericht."
      },
      {
        id: "ex4_v9",
        type: "mc",
        vraag: "Waarom was de Volkenbond vanaf het begin verzwakt?",
        opties: [
          "Omdat Duitsland, de Sovjet-Unie én de Verenigde Staten aanvankelijk geen lid waren.",
          "Omdat de Volkenbond geen kantoor had.",
          "Omdat alleen kleine eilanden lid mochten worden.",
          "Omdat Frankrijk weigerde mee te doen."
        ],
        antwoord: 0,
        uitleg: "Het Amerikaanse Congres stemde tegen lidmaatschap, en verliezers/communistische staten mochten eerst niet meedoen."
      },
      {
        id: "ex4_v10",
        type: "mc",
        vraag: "Wat wordt bedoeld met het 'zelfbeschikkingsrecht' van volkeren?",
        opties: [
          "Het recht van keizers om grenzen te wijzigen.",
          "Het recht van elk volk om zelf te bepalen bij welke staat hun gebied hoort en hoe het bestuurd wordt.",
          "Het recht van soldaten om hun eigen officier te kiezen.",
          "Het recht van de winnaar om alle fabrieken over te nemen."
        ],
        antwoord: 1,
        uitleg: "President Wilson vond dat elk volk het recht had op een eigen natiestaat en democratisch bestuur."
      },
      {
        id: "ex4_v11",
        type: "waaronwaar",
        vraag: "Door het zelfbeschikkingsrecht vallen multi-etnische keizerrijken zoals Oostenrijk-Hongarije en het Ottomaanse Rijk na WO1 uiteen.",
        antwoord: true,
        uitleg: "Waar! Volkeren zoals Tsjechen, Polen en Zuid-Slaven stichtten hun eigen onafhankelijke staten."
      },
      {
        id: "ex4_v12",
        type: "mc",
        vraag: "Welke van de volgende nieuwe staten ontstond NIET op de kaart van Europa na WO1?",
        opties: [
          "Tsjecho-Slowakije",
          "Joegoslavië",
          "Zwitserland",
          "Polen"
        ],
        antwoord: 2,
        uitleg: "Zwitserland bestond al eeuwen. Tsjecho-Slowakije, Joegoslavië en Polen waren nieuwe/herrezen staten."
      },
      {
        id: "ex4_v13",
        type: "mc",
        vraag: "Hoe werden de Arabische gebieden van het voormalige Ottomaanse Rijk na WO1 bestuurd door Frankrijk en Engeland?",
        opties: [
          "Als onafhankelijke koninkrijken.",
          "Als onderdeel van het Russische Rijk.",
          "Als provincies van Italië.",
          "Als mandaatgebieden namens de Volkenbond."
        ],
        antwoord: 3,
        uitleg: "Frankrijk en Groot-Brittannië kregen mandaat over gebieden zoals Syrië, Libanon, Irak en Palestina."
      },
      {
        id: "ex4_v14",
        type: "invul",
        vraag: "In welke grote Turkse havenstad in West-Anatolië (Smyrna) landde in 1919 een Grieks veroveringsleger?",
        antwoord: "Izmir|Smyrna|izmir|smyrna",
        uitleg: "Smyrna (het huidige Izmir) werd in 1919 bezet door Griekse troepen."
      },
      {
        id: "ex4_v15",
        type: "mc",
        vraag: "Wie leidde het Turkse nationale verzet tegen de verdeling van Anatolië en stichtte in 1923 de Republiek Turkije?",
        opties: [
          "Mustafa Kemal (Atatürk)",
          "Sultan Mehmed VI",
          "Enver Pasja",
          "Lenin"
        ],
        antwoord: 0,
        uitleg: "Generaal Mustafa Kemal verdreef de bezetters en werd de eerste president van de Republiek Turkije."
      },
      {
        id: "ex4_v16",
        type: "invul",
        vraag: "Welke erenaam, die 'Vader der Turken' betekent, gaf het Turkse parlement aan Mustafa Kemal?",
        antwoord: "Atatürk|Ataturk|ataturk|atatürk",
        uitleg: "Mustafa Kemal kreeg de achternaam Atatürk."
      },
      {
        id: "ex4_v17",
        type: "waaronwaar",
        vraag: "In 1923 werd tussen Griekenland en Turkije een verplichte 'volkenruil' afgesproken waarbij 1,2 miljoen Grieken en 500.000 Turken moesten verhuizen.",
        antwoord: true,
        uitleg: "Waar! De bevolkingsruil moest verdere etnische conflicten tussen natiestaten voorkomen."
      },
      {
        id: "ex4_v18",
        type: "mc",
        vraag: "Wat wordt bedoeld met het begrip 'etnische zuivering'?",
        opties: [
          "Het schoonmaken van historische gebouwen.",
          "Gewelddadige acties om een specifieke etnische groep uit een gebied te verdrijven of te verminderen.",
          "Het verplicht aanleren van een vreemde taal.",
          "Het sluiten van grenzen voor toeristen."
        ],
        antwoord: 1,
        uitleg: "Etnische zuivering omvat geweld, moord en verjaging om een gebied etnisch 'zuiver' te maken."
      },
      {
        id: "ex4_v19",
        type: "waaronwaar",
        vraag: "Oostenrijk wilde zich na WO1 graag aansluiten bij Duitsland, maar de Geallieerden verboden deze aansluiting streng.",
        antwoord: true,
        uitleg: "Waar! De Geallieerden wilden voorkomen dat Duitsland door de toevoeging van Oostenrijk te machtig zou worden."
      },
      {
        id: "ex4_v20",
        type: "open",
        vraag: "Leg uit waarom het Verdrag van Versailles van 1919 volgens veel historici nieuwe spanningen in Europa veroorzaakte in plaats van blijvende vrede.",
        sleutelwoorden: [
          "alleenschuld/vernedering/Duitsland/herstelbetalingen",
          "zelfbeschikkingsrecht/minderheden/Volkenbond"
        ],
        minTreffers: 1,
        modelantwoord: "Duitsland voelde zich diep vernederd door de alleenschuld, het zware gebiedsverlies en de onbetaalbare herstelbetalingen. Bovendien werd het zelfbeschikkingsrecht niet eerlijk toegepast (miljoenen Duitstaligen kwamen in nieuwe staten als Tsjecho-Slowakije en Polen terecht), wat leidde tot revanchisme en nieuwe etnische conflicten.",
        uitleg: "De harde straf voor Duitsland en de ongelijkmatige grenslijnen legden de kiem voor de Tweede Wereldoorlog."
      }
    ]
  });
})();
