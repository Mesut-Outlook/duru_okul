/* =========================================================
   Duru's Geschiedenis (HAVO 3) — Proeftoets 3 (20 vragen)
   Hoofdstuk 1: De Eerste Wereldoorlog — 1.3 Revolutie in Rusland & Sovjet-Unie
   ========================================================= */
(function () {
  "use strict";

  DURU.registerExamen({
    id: "ex-h3-geschiedenis-3",
    titel: "Proeftoets 3 — 1.3 Revolutie in Rusland & Het ontstaan van de Sovjet-Unie",
    vak: "Geschiedenis · Hoofdstuk 1",
    hoofdstuk: 1,
    hoofdstukTitel: "Hoofdstuk 1 — De Eerste Wereldoorlog (1900–1920)",
    icoon: "🛠️",
    duurMin: 20,
    vragen: [
      {
        id: "ex3_v1",
        type: "mc",
        vraag: "Wie was de laatste tsaar van het Russische Rijk die in 1917 na massale opstanden moest aftreden?",
        opties: [
          "Nicolaas II",
          "Peter de Grote",
          "Alexander III",
          "Jozef Stalin"
        ],
        antwoord: 0,
        uitleg: "Tsaar Nicolaas II trad in februari 1917 af."
      },
      {
        id: "ex3_v2",
        type: "waaronwaar",
        vraag: "Vóór 1917 bestond ongeveer 80 procent van de Russische bevolking uit arme, analfabete boeren die door de adel werden onderdrukt.",
        antwoord: true,
        uitleg: "Waar! Rusland was amper geïndustrialiseerd en de overgrote meerderheid van het volk leefde in armoede."
      },
      {
        id: "ex3_v3",
        type: "mc",
        vraag: "Wat gebeurde er tijdens 'Bloedige Zondag' in 1905 in Sint-Petersburg?",
        opties: [
          "Lenin werd tot premier gekozen.",
          "Het leger schoot honderden ongewapende arbeiders dood die hulp vroegen bij het Winterpaleis van de tsaar.",
          "Het Russische leger versloeg Japan.",
          "De Sovjet-Unie werd gesticht."
        ],
        antwoord: 1,
        uitleg: "Soldaten openden het vuur op vredige betogers bij het Winterpaleis, wat leidde tot grote verontwaardiging."
      },
      {
        id: "ex3_v4",
        type: "invul",
        vraag: "Hoe heette het Russische parlement dat door tsaar Nicolaas II in 1905 werd ingesteld, maar amper macht kreeg?",
        antwoord: "Doema|de Doema|doema",
        uitleg: "De Doema was het parlement, maar de tsaar hield alle echte macht zelf."
      },
      {
        id: "ex3_v5",
        type: "mc",
        vraag: "Wat was het directe gevolg van de Februarirevolutie in 1917?",
        opties: [
          "Rusland veroverde Berlijn.",
          "Lenin verbood alle godsdiensten.",
          "Tsaar Nicolaas II trad af, Rusland werd een republiek en de Doema vormde een voorlopige regering.",
          "Duitsland gaf zich over."
        ],
        antwoord: 2,
        uitleg: "De Februarirevolutie maakte van het tsarenrijk een republiek met een voorlopige regering."
      },
      {
        id: "ex3_v6",
        type: "invul",
        vraag: "Welke Nederlandse vertaling hoort bij het Russische woord 'sovjet'?",
        antwoord: "raad|raden",
        uitleg: "Een sovjet is een raad van gekozen arbeiders, boeren of soldaten."
      },
      {
        id: "ex3_v7",
        type: "waaronwaar",
        vraag: "Na de Februarirevolutie stopte de voorlopige regering in Rusland direct met de Eerste Wereldoorlog.",
        antwoord: false,
        uitleg: "Onwaar! De voorlopige regering zette de oorlog voort, wat tot enorme woede bij het volk en de soldaten leidde."
      },
      {
        id: "ex3_v8",
        type: "mc",
        vraag: "Hoe brachten de Duitsers de radicale communist Lenin in 1917 terug naar Rusland?",
        opties: [
          "Met een Duitse U-boot.",
          "Met een Duits vliegdekschip.",
          "In een vermomming als Britse soldaat.",
          "In het geheim met een speciale trein vanuit Zwitserland."
        ],
        antwoord: 3,
        uitleg: "Duitsland hoopte dat Lenin de chaos in Rusland zou vergroten, zodat Rusland uit de oorlog zou stappen."
      },
      {
        id: "ex3_v9",
        type: "invul",
        vraag: "Hoe noemde Lenin zijn radicale communistische aanhangers, wat 'zij die de meerderheid hebben' betekent?",
        antwoord: "bolsjewieken|bolsjewiek",
        uitleg: "Lenin noemde zijn volgers bolsjewieken."
      },
      {
        id: "ex3_v10",
        type: "mc",
        vraag: "Welke populaire leus gebruikte Lenin om aanhang te winnen onder het Russische volk?",
        opties: [
          "Vrede, Brood en Land!",
          "Geld, Vrijheid en God!",
          "Arbeid, Orde en Koning!",
          "Gelijkheid, Broederschap en Kroon!"
        ],
        antwoord: 0,
        uitleg: "Lenin beloofde vrede voor de soldaten, brood voor de steden en land voor de boeren."
      },
      {
        id: "ex3_v11",
        type: "waaronwaar",
        vraag: "Tijdens de Oktoberrevolutie van 1917 plegen de bolsjewieken onder leiding van Lenin een gewapende staatsgreep.",
        antwoord: true,
        uitleg: "Waar! In oktober 1917 bezetten de bolsjewieken de overheidsgebouwen in Petrograd."
      },
      {
        id: "ex3_v12",
        type: "mc",
        vraag: "Wat hield de 'dictatuur van het proletariaat' in die Lenin vestigde?",
        opties: [
          "Een democratie waarin alle burgers wekelijks stemden.",
          "Een alleenheerschappij waarin de communistische partij in naam van de arbeidersklasse alle macht bezat.",
          "Een regering geleid door de adel en kerk.",
          "Een staat zonder enige regering."
        ],
        antwoord: 1,
        uitleg: "Lenin maakte van Rusland een communistische eenpartijstaat zonder politieke oppositie."
      },
      {
        id: "ex3_v13",
        type: "mc",
        vraag: "Waarvoor stonden de hamer en de sikkel in het communistische symbool?",
        opties: [
          "Hamer = bouwvakkers; Sikkel = vissers",
          "Hamer = militairen; Sikkel = doktoren",
          "Hamer = fabrieksarbeiders; Sikkel = boeren",
          "Hamer = spoorwegen; Sikkel = leraren"
        ],
        antwoord: 2,
        uitleg: "De hamer verbeeldde de arbeider en de sikkel de boer."
      },
      {
        id: "ex3_v14",
        type: "invul",
        vraag: "Hoe heette de meedogenloze communistische geheime dienst die door Lenin werd opgericht (later bekend als KGB)?",
        antwoord: "Tsjeka|de Tsjeka|tsjeka",
        uitleg: "De Tsjeka spoorde tegenstanders op en vermoordde of verbanne hen."
      },
      {
        id: "ex3_v15",
        type: "waaronwaar",
        vraag: "In 1918 werden ex-tsaar Nicolaas II en zijn complete gezin op bevel van de bolsjewieken doodgeschoten.",
        antwoord: true,
        uitleg: "Waar! De tsarenfamilie werd geëxecuteerd om te voorkomen dat ze bevrijd zouden worden."
      },
      {
        id: "ex3_v16",
        type: "mc",
        vraag: "Wie vochten er tegen elkaar in de bloedige Russische Burgeroorlog (1918–1922)?",
        opties: [
          "De boeren tegen de fabrieksarbeiders.",
          "De Russen tegen de Chinezen.",
          "De Fransen tegen de Britse troepen.",
          "Het communistische Rode Leger tegen de 'Witten' (tsaaraanhangers, adel en democraten)."
        ],
        antwoord: 3,
        uitleg: "De burgeroorlog ging tussen de communistische 'Roden' en hun tegenstanders, de 'Witten'."
      },
      {
        id: "ex3_v17",
        type: "invul",
        vraag: "In welk jaar werd het verbond van sovjetrepublieken, de Sovjet-Unie (USSR), officieel gesticht?",
        antwoord: "1922",
        uitleg: "In 1922 werd de Sovjet-Unie officieel opgericht."
      },
      {
        id: "ex3_v18",
        type: "mc",
        vraag: "Waarom werd de naam van de stad Sint-Petersburg in 1914 veranderd in Petrograd?",
        opties: [
          "Omdat Sint-Petersburg tijdens de oorlog met Duitsland te Duits klonk.",
          "Ter ere van tsaar Nicolaas II.",
          "Omdat de stad door de Fransen werd veroverd.",
          "Op bevel van de kerk."
        ],
        antwoord: 0,
        uitleg: "Petrograd is de Russisch/Slavische naam voor de stad van Peter."
      },
      {
        id: "ex3_v19",
        type: "waaronwaar",
        vraag: "Na de dood van Lenin in 1924 werd Petrograd omgedoopt tot Leningrad.",
        antwoord: true,
        uitleg: "Waar! De Sovjet-regering doopte de stad om ter ere van Lenin."
      },
      {
        id: "ex3_v20",
        type: "open",
        vraag: "Leg in je eigen woorden uit wat het verschil was tussen de Februarirevolutie en de Oktoberrevolutie van 1917 in Rusland.",
        sleutelwoorden: [
          "tsaar/aftreden/republiek/voorlopige regering/Nicolaas",
          "Lenin/bolsjewieken/staatsgreep/communisme/dictatuur"
        ],
        minTreffers: 1,
        modelantwoord: "Bij de Februarirevolutie trad tsaar Nicolaas II af en werd Rusland een democratische republiek met een voorlopige regering. Bij de Oktoberrevolutie pleegden Lenin en de bolsjewieken een gewapende staatsgreep en maakten ze van Rusland een communistische dictatuur (eenpartijstaat).",
        uitleg: "Februari = einde tsarenrijk & republiek; Oktober = staatsgreep bolsjewieken & communisme."
      }
    ]
  });
})();
