/* =========================================================
   Duru's Geschiedenis Academie — Oefentoetsen (Examen-modus)
...
(function () {
  ...
  var EX_SLEUTEL = "duru_2627_geschiedenis_examens_v1";
  function laadEx() {
    var d = null;
  ...
  DURU.renderExamenLijst = function () {
    ...
    Object.keys(groepen).forEach(function (hfTitel) {
      ...
      h += '</div></div></div>'; // Einde examen-lijst, hf-body en hf-accordion-card
    });

    DURU.toggleHoofdstukEx = function(idx) {
      var card = document.getElementById("hf-ex-card-" + idx);
      var content = document.getElementById("hf-ex-content-" + idx);
      var label = document.getElementById("hf-ex-label-" + idx);
      if (!content) return;
