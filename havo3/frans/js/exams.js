/* =========================================================
   Duru's Frans Academie — Oefentoetsen (Examen-modus)
...
(function () {
  ...
  var EX_SLEUTEL = "duru_2627_frans_examens_v1";
  function laadEx() {
    var d = null;
  ...
  DURU.renderExamenLijst = function () {
    ...
    h += '<div class="sectie-titel"><h3>📝 Oefentoetsen per Unité</h3><div class="lijn"></div></div>';
    h += '<p style="margin:0 4px 16px;color:var(--grijs)">Doe een toets op tijd, net als op school! Je krijgt aan het eind je cijfer én bij elke vraag te zien <b>hoe je het moet doen</b>. Klik op een Unité om de toetsen in of uit te vouwen.</p>';
    ...
    DURU.examens.forEach(function (ex) {
      ...
      if (!chaptersMap[hfNr]) {
        chaptersMap[hfNr] = {
          meta: { nr: hfNr, titel: hfNr === 999 ? "Overige toetsen" : (ex.hoofdstukTitel || ("Unité " + hfNr)), icoon: ex.icoon || "🇫🇷", intro: "" },
          examens: []
        };
      }
    ...
    Object.keys(chaptersMap).sort(function(a, b) { return parseInt(a, 10) - parseInt(b, 10); }).forEach(function (hfNr) {
      ...
      var avgScore = completedExams > 0 ? (Math.round((1 + (sumPcts / completedExams) / 100 * 9) * 10) / 10).toFixed(1).replace(".", ",") : null;

      h += '<details class="chapter-accordion" id="ch-acc-' + meta.nr + '" open>';
      h += '<summary class="chapter-header">' +
        '<div class="ch-icon">' + (meta.icoon || "🇫🇷") + '</div>' +
        '<div class="ch-info">' +
          '<span class="ch-badge">' + (meta.nr === 999 ? 'Extra' : 'Unité ' + meta.nr) + '</span>' +
      ...
      h += '<div class="chapter-content"><div class="examen-lijst">';
