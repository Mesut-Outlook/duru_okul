...
(function () {
  ...
  var SLEUTEL = "duru_2627_engels_v1";
  var P = laad();
  ...
  var BADGES = [
    ...
    { id: "prof", ico: "🎓", naam: "Engels-professor", check: function () { return P.xp >= 500; } },
  ];
  ...
  function renderHome() {
    ...
    html += '<section class="hero view">' +
      ...
      '<p>Welkom in jouw eigen Engels-academie. Leer alle <b>grammatica</b>, <b>vocabulaire</b> en <b>leesvaardigheid</b> voor HAVO 3 ' +
      'met ' + (totaalExamens > 0 ? (totaalExamens * 20) : totaalVragen) + ' vragen verdeeld over alle 6 hoofdstukken. Klik op een hoofdstuk om de toetsen te bekijken. Jij gaat die toets máken! 💪</p>' +
      '<div class="hero-cta">' +
      '<button class="btn oranje" onclick="DURU.gaNaar(\'examens\')">📝 Oefentoetsen per Hoofdstuk</button>' +
    ...
    // Oefentoetsen-sectie per Hoofdstuk (Collapsible)
    if (DURU.examens && DURU.examens.length) {
      ...
      html += '<p style="margin:0 4px 14px;color:var(--grijs)">Kies hieronder een Hoofdstuk. Elk hoofdstuk bevat 5 gerichte proeftoetsen (Vocabulary, Grammar, Stones & Communication, Reading en Chapter Test). Klik op een hoofdstuk om deze in of uit te klappen.</p>';
    ...
    if (DURU.hoofdstukken && DURU.onderwerpen && DURU.onderwerpen.length > 0) {
      DURU.hoofdstukken.forEach(function (h) {
        ...
        html += '<summary class="chapter-header">' +
          ...
          '<div class="ch-info">' +
            '<span class="ch-badge">Theorie Hoofdstuk ' + h.nr + '</span>' +
    ...
    html += '<div class="footer">Gemaakt met 🇬🇧 en 💜 voor Duru · Engels HAVO 3</div>';
    app.innerHTML = html;
    updateStats();
  }
