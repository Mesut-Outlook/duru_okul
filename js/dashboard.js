...
(function () {
  ...
  // ── Render Card/Bars Component ────────────────────────────
  function renderSubjectPerformance(attempts) {
    var grid = document.getElementById("subject-progress-grid");
    if (!grid) return;

    var subjectsList = [
      { id: "natuurkunde", title: "Natuurkunde (NASK)", icon: "⚛️", colorHex: "var(--blauw)" },
      { id: "wiskunde", title: "Wiskunde", icon: "⚖️", colorHex: "var(--teal)" },
      { id: "economie", title: "Economie", icon: "💶", colorHex: "var(--groen)" },
      { id: "nederlands-spelling", title: "Spelling & Grammatica", icon: "✍️", colorHex: "var(--oranje)" },
      { id: "nederlands-begrijpend", title: "Begrijpend Lezen", icon: "🧠", colorHex: "var(--oranje)" }
    ];

    var html = "";

    subjectsList.forEach(function (sub) {
      var subAttempts = attempts.filter(function (att) { return att.vakId === sub.id; });
      var count = subAttempts.length;
      var avgGrade = 0;
      var maxGrade = 0;

      if (count > 0) {
        var sum = 0;
        subAttempts.forEach(function (att) {
          sum += att.cijfer;
          if (att.cijfer > maxGrade) maxGrade = att.cijfer;
        });
        avgGrade = sum / count;
      }

      // Bar width as percentage of 10
      var pctWidth = count > 0 ? (avgGrade / 10) * 100 : 0;
      var avgStr = count > 0 ? avgGrade.toFixed(1).replace(".", ",") : "-";
      var maxStr = count > 0 ? maxGrade.toFixed(1).replace(".", ",") : "-";

      var detailsText = "Nog geen toetsen gemaakt";
      if (count === 1) {
        detailsText = "1 toets gemaakt · Hoogste: " + maxStr;
      } else if (count > 1) {
        detailsText = count + " toetsen gemaakt · Hoogste: " + maxStr;
      }

      html += '<div class="sub-analytics-card">' +
                '<div class="sub-analytics-card__header">' +
                  '<span>' + sub.icon + ' ' + sub.title + '</span>' +
                  '<span style="color:' + sub.colorHex + '; font-weight: bold;">' + avgStr + ' <span style="font-size:11px; color:var(--grijs-licht); font-weight: normal;">/ 10</span></span>' +
                '</div>' +
                '<div class="sub-analytics-card__bar-bg">' +
                  '<div class="sub-analytics-card__bar-fill" style="width:' + pctWidth + '%; background:' + sub.colorHex + '"></div>' +
                '</div>' +
                '<div class="sub-analytics-card__details">' +
