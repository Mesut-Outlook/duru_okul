...
def check_and_save(exam_id, exam_num, title, icon, questions):
    ...
    for q in open_q:
        assert q["minTreffers"] <= len(q["sleutelwoorden"])
        for kw in q["sleutelwoorden"]:
            for alt in kw.split("/"):
                alt_clean = alt.strip().lower()
                assert alt_clean not in q["vraag"].lower(), f"Keyword '{alt_clean}' found in question text: {q['vraag']}"
...
"""
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"✓ Succesvol aangemaakt: {filepath} ({len(questions)} vragen, MC 3-3-3-3, Waar/Onwaar 2-2)")
