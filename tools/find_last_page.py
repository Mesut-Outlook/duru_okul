...
def find_last_page(page, book_id, name):
    print(f"Finding last page of {name}...")
    for p in [170, 165, 160, 158, 156, 155, 154, 153]:
        url = f"https://apps.noordhoff.nl/se/content/book/{COURSE_ID_ED7}/ebook/{book_id}?page={p}"
        page.goto(url)
        time.sleep(2.5)
        clean_ui(page)
        shot = f"/tmp/last_{name}_{p}.png"
        page.screenshot(path=shot)
        txt = ocr_image(shot)
        lines = [l.strip() for l in txt.split('\n') if l.strip()]
        print(f"  {name} Page {p:3d} (URL {page.url}): {' | '.join(lines[:3])}")
...
def main():
    with sync_playwright() as p:
        ...
        find_last_page(page, BUCH_A_ID, "Buch_A")
        find_last_page(page, BUCH_B_ID, "Buch_B")
